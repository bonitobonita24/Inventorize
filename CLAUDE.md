...
>   Cline runs Phase 4 Part-by-Part (one Part per fresh session), then Phase 5–8
...
>
> Cline              → Building everything. Phase 4 Part-by-Part, Phase 5–8 on human trigger.
>                      Reads .clinerules. Phase 4 = Part-by-Part (Rule 24) — one Part per session.
>                      Human says "Start Phase 5", "Start Phase 6" after each completes.
>                      Default model: MiniMax M2.5 via OpenRouter (free tier). See Tool Setup Guide for full model routing.
>
...
MODELS:
  planning:   [model name]
  execution:  [model name — default MiniMax M2.5 via Cline]
...
Every project skill file uses the cross-agent standard location so skills work in Cline,
Claude Code, GitHub Copilot, and VS Code without any path changes.
...
**Applies to all agents:** Cline, Claude Code, Copilot, Planning Assistant.
...
**Who:** Cline (Part-by-Part, human triggers each phase) | **Where:** VS Code — Cline panel
...
Step 3 — .clinerules (Cline reads this before every task)
  ...
  - Each Part runs in a SEPARATE Cline session. Never auto-chain parts.
  - Open the next part task file ONLY after the current part is fully complete and merged.
  - ALWAYS STOP after each Part. Output "✅ Part [N] complete. Open phase4-part[N+1].md in a NEW Cline session."
  ...
  If a Cline session ends or is closed before a Part completes:
  ...
  - After all 9 pass: STOP. Output "✅ Phase 5 complete. Say 'Start Phase 6' in a new Cline session."
...
Step 10 — .vscode/mcp.json (MCP entries — V10 + Context7 NEW V23)
  {
    "servers": {
      "socraticode": {
        "command": "npx",
  ...
  → Context7 fetches current Auth.js v5 docs before Cline generates any code.
...
Step 16 — Git init + STATE.md (NEW V14)
  ...
  MODELS:
    planning:   claude-code (Phase 2) or minimax-m2.5 (Cline)
  ...
  ```
  Write .gitignore entry: `.env`, `.env.local`, `.env.dev`, `.env.staging`, `.env.prod`, `CREDENTIALS.md` (never commit secrets)
  CREDENTIALS.md MUST be in .gitignore BEFORE Phase 3 writes it — this is non-negotiable.
...
Step 18 — Credential Collection Gate (NEW V23) ← BLOCKING: do not advance to Phase 1 until this step is complete

  After Step 17, STOP all file generation. Output the following questions to the human
  and wait for answers before proceeding. This step writes CREDENTIALS.md in full.
  CREDENTIALS.md must be complete before any phase can begin.

  OUTPUT THIS TO HUMAN — wait for all answers before continuing:
  ```
  ════════════════════════════════════════════════════════════════
  ⚠️  CREDENTIAL COLLECTION — Bootstrap Step 18
  ════════════════════════════════════════════════════════════════
  I need to collect access credentials before any code is generated.
  These will be written to CREDENTIALS.md (gitignored — never committed).
  Answer each section below. Type SKIP for any optional item you want to set up later.

  ── SECTION 1: GitHub ──────────────────────────────────────────
  1a. GitHub username (for gh CLI and git remote):
  1b. Do you have the GitHub CLI (gh) installed? [yes / no / will install]
  1c. GitHub repo name for this project (e.g. powerbyte-erp):
  1d. GitHub repo visibility: [public / private]
  1e. GitHub Personal Access Token with repo + workflow scope:
      (Get one: github.com/settings/tokens → New token (classic) → select repo, workflow)
      Token value:

  ── SECTION 2: Docker Hub ─────────────────────────────────────
  (Required if docker.publish: true in inputs.yml — skip if not using Docker pipeline)
  2a. Docker Hub username:
  2b. Docker Hub access token (NOT your password):
      (Get one: hub.docker.com → Account Settings → Security → New Access Token)
      Token value:

  ── SECTION 3: SMTP / Mail Server ────────────────────────────
  (Required for account registration, forgot password, OTP emails)
  3a. SMTP host (e.g. smtp.gmail.com / smtp.mailtrap.io):
  3b. SMTP port (usually 587 for TLS, 465 for SSL):
  3c. SMTP username / login email:
  3d. SMTP password or app password:
  3e. From address (e.g. noreply@yourapp.com):
  3f. From name (e.g. "Nucleus ERP"):
  ...
  AFTER HUMAN PROVIDES ANSWERS — execute all of the following:
  ...
  B) Write the complete CREDENTIALS.md — ALL sections filled with ACTUAL values:
     ...
     | Field                | Value                              |
     |----------------------|------------------------------------|
     | Username             | [from Section 1a]                  |
     | Repo name            | [from Section 1c]                  |
     | Repo URL             | https://github.com/[user]/[repo]   |
     | Visibility           | [public / private]                 |
     | Personal Access Token| [from Section 1e]                  |
     | Token scopes         | repo, workflow                     |
     | gh CLI installed     | [yes / no]                         |

     Git remote: git remote add origin https://github.com/[user]/[repo].git
     ...
     | Field        | Value                             |
     |--------------|-----------------------------------|
     | Username     | [from Section 2a]                 |
     | Access Token | [from Section 2b]                 |
     ...
     ## 📧 SMTP (Email — account registration, forgot password, OTP)

     | Field        | Value               |
     |--------------|---------------------|
     | SMTP Host    | [from Section 3a]   |
     | SMTP Port    | [from Section 3b]   |
     | Username     | [from Section 3c]   |
     | Password     | [from Section 3d]   |
     | From address | [from Section 3e]   |
     | From name    | [from Section 3f]   |

     ⚠ This credential is provided by you — not generated by the AI.
     For dev: MailHog runs locally (no real SMTP needed) — port shown in .env.dev.
     For staging/prod: use the credentials above.
     ...
     | Service | Key Name | Value | Environment |
     |---------|----------|-------|-------------|
     | [from Section 5a — fill each key-value pair here] | | | |

     Add more rows as new third-party services are integrated.
     ...
     Run Bootstrap Step 18 or fill manually to regenerate credentials.
     ```
  ...
  D) Output the "Bootstrap complete" message below. DO NOT output it before Step 18 is finished.
     CREDENTIALS.md must be fully written before Phase 2 begins. Phase 1 (dev environment setup) is optional and may be skipped if already configured.

After Cline finishes all 18 steps, OUTPUT THE FOLLOWING TEXT TO THE HUMAN. Do not execute these instructions yourself — they are for the human to read:
```
✅ Bootstrap complete — credentials collected and written to CREDENTIALS.md.
...
□ CREDENTIALS.md exists in the project root
  - If missing: STOP. Output: "⚠ Bootstrap Step 18 has not run. CREDENTIALS.md is missing.
    Open Cline, paste the master prompt, and type 'Bootstrap' to complete setup first."
...
> After confirming, hand off to Cline for Phase 4 onwards — Part-by-Part, human triggers each phase.
...
→ then Phase 3 generates spec files → then hand off to Cline for Phase 4 Part-by-Part.
```
  ...
  If PRODUCT.md Non-functional Requirements contains "accessibility: wcag_aa":
    → Add section to design-system/MASTER.md:
      "## Accessibility (WCAG 2.1 AA) — MANDATORY
       ...
       Run /web-interface-guidelines in Cline to audit UI code against these standards."
...
every gap, ambiguity, and missing decision that would force Cline to halt or guess during Phase 4.
...
Step 2 — Run the 4-category internal stress-test.
  ...
  AMBIGUITY
  □ Any workflow step where Cline cannot determine which file to create?
...
> **Open Cline and say "Start Phase 4". Cline runs Part 1. After each Part: STOP. Human opens next Part in a new session.**

---

## PHASE 4 — FULL MONOREPO SCAFFOLD
**Who:** Cline (Part-by-Part, fresh session each Part — Rule 24) | **Where:** VS Code — Cline panel

Each Part runs in a FRESH Cline session (Rule 24 — prevents context accumulation).
MiniMax M2.5 optimization: each Part stays under ~3,000 lines of context.
...
| What to say in Cline / Claude Code | What it does |
|---|---|
...
Cline generates this file alongside `docker-compose.pgadmin.yml` for each environment.
...
# ── Config (Cline fills these from inputs.yml during Phase 4 Part 7) ──
...
After Part 8 complete → STOP. Human manually triggers Phase 5: say "Start Phase 5" in a new Cline session. This is consistent with Rule 24 fresh-context discipline — never auto-chain phases.
...
Each phase runs in its own Cline session. Human triggers the next phase explicitly.
...
**Triggers:**
```
After Phase 4 Part 8 completes: say "Start Phase 5" in a new Cline session
After Phase 5 passes:           say "Start Phase 6" in a new Cline session
...
## PHASE 5 — VALIDATION
**Who:** Cline (human-triggered: "Start Phase 5") | **Where:** WSL2 terminal

Cline runs all 9 commands. Fixes every failure before proceeding.
...
**When all 9 pass → Phase 5 is complete. Output "✅ Phase 5 complete. Say 'Start Phase 6' in a new Cline session."**
Human triggers Phase 6 explicitly.

---

## PHASE 6 — START DOCKER SERVICES
**Who:** Cline (human-triggered: "Start Phase 6") or you manually | **Where:** WSL2 terminal
...
IF ANY item fails → Phase 6 = INCOMPLETE → fix → re-verify before reporting done
─────────────────────────────────────────────────────────
...
□ CHANGELOG_AI.md: entry written with this session's timestamp and Agent: CLINE
...
WRITING A CUSTOM SKILL:
  ...
  3. Test: say "Feature Update" describing a task matching the description line
     → Cline should load the skill. If not: tighten the description.
...
```
OVERVIEW:
  Context7 MCP injects current version-specific library documentation into Cline's context
...
SETUP (done once at bootstrap — already in .vscode/mcp.json):
  ...
  Verify it works: ask Cline "use context7 to look up next.js middleware" — you should see
  Context7 resolve the library and return current docs.

HOW TO USE — append "use context7" to any Cline task:

  EXAMPLE 1 — Auth.js v5 setup:
  "Set up Auth.js v5 Credentials provider with bcrypt password hashing. use context7"
  Context7 fetches /nextauthjs/next-auth docs for the v5 API before Cline writes any code.
  Without context7: Cline may use the v4 authorize() pattern, causing runtime errors.
...
MANUAL LOOKUP (without triggering full task):
  In Cline: "/context7:docs prisma soft delete"
...
EDGE CASE 1 — Phase 4 Part interrupted mid-session (session closed, crash, power loss):
  On next Cline session: read STATE.md → detect PHASE = "[Part N] PARTIAL"
...
COMMON GAPS BY CATEGORY:
  ...
  GAP 3:
    ...
    PROBLEM:  "Admin can manage users" is too vague — Cline cannot determine which operations apply
...
MID-PROJECT USE (after Phase 4 is already running):
  IF gaps found AFTER Phase 4 started:
    ...
    - Use "Feature Update" in Cline to implement changes on top of existing scaffold.
...
IF docker.publish: false (opted out):
  ...
  To enable later: set docker.publish: true in inputs.yml → Feature Update → Cline generates
...
Model options:
```
DEFAULT: OpenRouter → minimax/minimax-m2.5        ★ Free tier. Best value for execution phases.
         MiniMax M2.5: upgraded code quality + native spec-following. Recommended for ALL phases.
...
```
V19 model routing (locked in inputs.yml):
  planning:   claude-code (Phase 2 interview)
  execution:  minimax-m2.5 (Phase 4-8, Feature Updates — default free tier)
...
Usage: append "use context7" to any Cline task that involves external library code.
...
readable by Cline, Claude Code, GitHub Copilot, and VS Code equally.
...
Never load all skills at once — contextual loading keeps context lean for MiniMax M2.5.
...
Files named: `Master Prompt v25.md`, `v23.md`, etc.
All 7 files in the complete set always share the same version number.
...
When a Cline session ends before a Phase 4 Part completes:
...
────────────  ──────────────────────────  ──────────────────────────────────────
Claude Code   Phase 0 (Bootstrap read),   IF asked to run Phase 4–8 scaffold
              Phase 2, 2.5, 2.6, 2.7, 3  work → output: "⚠ Phase [N] is a Cline
                                          phase. Open Cline and run from there."
Cline         Phase 4, 5, 6, 6.5,        IF asked to rewrite PRODUCT.md →
              7, 7R, 8, Resume,           output: "⚠ PRODUCT.md is human-owned.
              GovSync, Retro, Handoff     Describe the change and I will list
                                          the exact edit needed."
Copilot       Inline autocomplete,        IF generating governance docs or
              PR review only             phase scaffold → defer to Cline
Planning      PRODUCT.md writing only    IF asked to generate inputs.yml,
Assistant     (no code, no config,       code, or scaffold → output: "I only
              no inputs.yml)             produce PRODUCT.md. Take this spec
                                          to your project and run Phase 2."
```
...
→ Phase 0      — Bootstrap (Cline automated — type "Bootstrap") — copy PRODUCT.md into docs/ after this completes
...
→ Phase 5      — Validation — CLINE (human trigger: "Start Phase 5" after Phase 4 Part 8)
→ Phase 6      — Docker + Visual QA — CLINE (human trigger: "Start Phase 6" after Phase 5)
